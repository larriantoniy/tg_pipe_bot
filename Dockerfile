...
RUN go build -o tg_pipe_bot ./cmd/pipebot
...
COPY --from=go-builder   /app/tg_pipe_bot               /usr/local/bin/tg_pipe_bot
# создаём каталог для конфигураций
RUN mkdir -p /etc/tg_pipe_bot

# копируем туда файл dev.yaml
COPY --from=go-builder /app/config/dev.yaml /etc/tg_pipe_bot/dev.yaml
